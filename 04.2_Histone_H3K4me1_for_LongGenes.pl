...
open(INB,"Epigenome/mENCODE_ChIP_bedgraph/H3K4me1_RenLab_8w.txt.$chr")||die("Can't open INB:$!\n");
...
while(<INC>)
{
 ...
 if(/^ENSMUSG\d+\_[\w\.\-\(\)]+\t[\w\.\-\(\)]+\t(chr\w\w?)\:(\d+)\-(\d+)\:(ENSMUSG\d+\_[\w\.\-\(\)]+)\:\w+\:([+|-])$/)
 {
  if($1 eq $chr) ##  Long Genes on $chr
  {
   ...
   open(OUT, ">Epigenome/value/$gene_id\_H3K4me1.txt")||die("Can't write OUT:$!\n");
